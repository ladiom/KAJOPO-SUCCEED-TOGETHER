...
class OpportunityManager {
     ...
     */
    async filterOpportunities(filters) {
        let opportunities = [];
        
        // Try to get opportunities from database first
        if (window.dbManager && await window.dbManager.ensureInitialized()) {
            try {
                const result = await window.dbManager.getOpportunities();
                ...
                console.error('Error getting opportunities from database:', error);
                opportunities = this.getAllOpportunities();
            }
        } else {
            // Use localStorage if database not available
            opportunities = this.getAllOpportunities();
        }
...
async function loadDynamicOpportunities() {
    ...
    // Show loading state
    container.innerHTML = '<div class="col-span-full text-center py-8"><div class="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto"></div><p class="mt-4 text-gray-600">Loading opportunities...</p></div>';
    ...
    try {
        ...
        console.log('Using localStorage fallback for opportunities');
        ...
        // Show error state with fallback to localStorage
        const opportunities = opportunityManager.getAllOpportunities();
        container.innerHTML = '';
        ...
        if (typeof errorHandler !== 'undefined') {
            errorHandler.handleError(error, 'Load opportunities');
        }
    }
}
...
function updateResultsCount(count) {
    // If no count provided, calculate total from all opportunities
    if (count === undefined) {
        const dynamicCount = JSON.parse(localStorage.getItem('kajopo_opportunities') || '[]').length;
        const staticCount = 3; // Number of hardcoded opportunities
        count = dynamicCount + staticCount;
    }
    
    // Update the opportunity count display
    const opportunityCountElement = document.getElementById('opportunityCount');
    if (opportunityCountElement) {
        opportunityCountElement.textContent = count;
    }
    
    // Also update any other results count elements for backward compatibility
    const resultsElements = document.querySelectorAll('span.font-medium');
    if (resultsElements.length >= 3) {
        resultsElements[2].textContent = count;
    }
}
