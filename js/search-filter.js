...
const searchFilter = {
    ...
    updateResultsCount(filters) {
        try {
            // Simulate results count calculation
            let count = 247; // Base count
            
            if (filters.search) count = Math.floor(count * 0.6);
            if (filters.category) count = Math.floor(count * 0.7);
            if (filters.type) count = Math.floor(count * 0.8);
            if (filters.region) count = Math.floor(count * 0.9);
            ...
            if (typeof errorHandler !== 'undefined') {
                ...
                if (hasFilters) {
                    errorHandler.showInfo(`Found ${count} matching opportunities`);
                }
            }
        } catch (error) {
